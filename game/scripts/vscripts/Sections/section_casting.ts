...
const start = (complete: () => void) => {
    ...
    const pugna = CreateUnitByName("npc_dota_hero_pugna",heroLoc,true,undefined,undefined,DotaTeam.BADGUYS)
    pugna.SetHealth(1);
    pugna.AddNewModifier(undefined,undefined,"modifier_tutorial_disable_healing",{});
    const disarmModifier = pugna.AddNewModifier(pugna,pugna.FindAbilityByName("modifier_pugna_decrepify"),"modifier_pugna_decrepify", {})

    graph = tg.seq(
        tg.completeOnCheck(()=> !ability.IsCooldownReady() && !pugna.IsAlive(),.1),
...
export const section_casting = new tut.FunctionalSection("Section_Casting", start, resetState, stop)
